...
def get_change_interpretation(stats, date1, date2, region, src1, src2,
    ...
    prompt = (
        ...
        f"compare spatially, and whether the change is uniform or patchy (use std dev).\n\n"
        f"Section 2 — Causes: The most likely drivers. Consider seasonal effects "
        f"(are the two dates in the same season?), land use, rainfall, agricultural cycles, "
        f"deforestation, or climate anomalies. Give specific hypotheses for this region.\n\n"
        f"Section 3 — Application: One concrete decision this data could support. "
        f"Name a specific stakeholder (government agency, agricultural ministry, NGO, "
        f"insurer) and describe exactly how they would use these findings.\n\n"
        f"Section 4 — Limitation: What NDVI change detection cannot tell us. "
        f"Be specific about the data source, resolution, and the difference between "
        f"phenological change and permanent land cover change.\n\n"
        f"Write in plain language. Use section headings in bold. Be direct and thorough."
    )
